      ...
      chrome.runtime.sendMessage({ type: 'GS4PM_BROADCAST', tabId, message }, (result) => {
        if (chrome.runtime.lastError) {
          ...
          chrome.tabs.sendMessage(tabId, message, () => {
            if (chrome.runtime.lastError) {
              console.log('[GS4PM Filter][sidepanel] No receiver for', message.type, '-', chrome.runtime.lastError.message);
            }
          });
          return;
        }
        ...
        if (result && typeof result === 'object' && result.successCount === 0) {
          console.log('[GS4PM Filter][sidepanel] Broadcast had no receivers for', message.type, result);
        }
      });
      return;
    }
...
function focusAddCustomerInput() {
  // `manageSection` is a <details> in the side panel UI.
...
toggleTaggingBtn.addEventListener('click', () => {
  ...
  if (!taggingEnabled) {
    ...
    if (!tagCustomer || tagCustomer === 'ALL') {
      console.log('[GS4PM Filter][sidepanel] Cannot enable tagging – no customer selected.');
      taggingEnabled = false;
      saveTaggingEnabled(false);
      updateTaggingButtonUI();
      return;
    ...
    console.log('[GS4PM Filter][sidepanel] Enabling tagging for customer', tagCustomer);
    ...
    console.log('[GS4PM Filter][sidepanel] Disabling tagging');
    sendMessageToActiveTab({ type: 'STOP_TAGGING' });
  }
});

// Initial load; re-sync when storage changes (workspace bar, other tabs, context menu).
wireFooter();
